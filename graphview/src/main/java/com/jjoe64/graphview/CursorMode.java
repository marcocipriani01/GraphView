...
public class CursorMode {
    ...
    protected void drawLegend(Canvas canvas) {
        ...
        canvas.drawText(mGraphView.getGridLabelRenderer().getLabelFormatter().formatLabel(mCurrentSelectionX, true), lLeft+mStyles.padding, lTop+mStyles.padding/2f+mStyles.textSize, mTextPaint);
        ...
        for (Map.Entry<BaseSeries<?>, DataPointInterface> entry : mCurrentSelection.entrySet()) {
            ...
            canvas.drawText(getTextForSeries(entry.getKey(), entry.getValue()), lLeft+mStyles.padding+shapeSize+mStyles.spacing, lTop+mStyles.padding/2f+mStyles.textSize+(i*(mStyles.textSize+mStyles.spacing)), mTextPaint);
            i++;
        }
    }
