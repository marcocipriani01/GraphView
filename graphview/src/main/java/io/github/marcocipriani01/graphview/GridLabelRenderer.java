...
public class GridLabelRenderer {
    ...
    public enum GridStyle {
        ...
        public boolean drawVertical() { return this == BOTH || this == VERTICAL; }
        public boolean drawHorizontal() { return this == BOTH || this == HORIZONTAL; }
    }
    ...
    protected boolean adjustVerticalSecondScale() {
        ...
        if (mStepsVerticalSecondScale != null && mStepsVerticalSecondScale.size() > 1) {
            ...
            if (oldSteps > 0) {
                ...
                if (!Double.isNaN(newSteps) && shouldChange && numStepsNew <= numVerticalLabels) {
                    exactSteps = newSteps;
                } else {
    ...
    protected boolean adjustHorizontal(boolean changeBounds) {
        ...
        if (isHumanRoundingX()) {
            ...
            if (oldSteps > 0) {
                ...
                if (!Double.isNaN(newSteps) && shouldChange && numStepsNew <= numHorizontalLabels) {
                    exactSteps = newSteps;
                } else {
                    // try to stay to the old steps
                    exactSteps = oldSteps;
